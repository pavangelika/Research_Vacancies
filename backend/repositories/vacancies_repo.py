            ...
            items.append(
                {
                    ...
                    "work_format": row[16],
                    "apply_alternate_url": row[17],
                    "send_resume": bool(row[18]) if row[18] is not None else False,
                    "role_id": str(row[19]) if row[19] is not None else None,
                    "employer_accredited": bool(row[20]) if row[20] is not None else None,
                    "employer_trusted": bool(row[21]) if row[21] is not None else None,
                    "employer_rating": str(row[22]) if row[22] is not None else None,
                    "employer_url": row[23],
                }
            )
        return items
