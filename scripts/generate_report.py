...
def render_report(data):
    ...
    roles = [{'name': row[0], 'count': row[1]} for row in data]
...
def save_report(html_content):
    ...
    logging.info(f"Report saved to {output_dir}/report.html")
...
def main():
    # Загружаем маппинг ролей из JSON
...
#     cur.close()
#     conn.close()
#     return rows  # список кортежей (role, count)
#
# def render_report(data):
#     env = Environment(loader=FileSystemLoader('templates'))
#     template = env.get_template('report_template.html')
...
#     if not data:
#         logging.warning("No data found. Empty report will be generated.")
#         # Создадим пустой список, чтобы шаблон не сломался
#         data = []
#     else:
#         logging.info(f"Found {len(data)} roles.")
#
#     html = render_report(data)
#     save_report(html)
#     copy_styles()
